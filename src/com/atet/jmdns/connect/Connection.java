package com.atet.jmdns.connect;
...
public class Connection {
	...
	/**
	 * returns local port used in
	 * {@link me.alwx.localcommunication.connection.ConnectionWrapper}
	 * 
	 * @return port
	 */
				...
				public void run() {
					try {
						...
						while (!Thread.currentThread().isInterrupted()) {
							...
							connectToServer(mSocket.getInetAddress(),
									mSocket.getPort());
						}
					} catch (IOException e) {
			...
			public void run() {
				...
				try {
					...
					while (!Thread.currentThread().isInterrupted()) {
						...
						if (messageStr != null) {
							Log.d(DEBUG_TAG, "Read from the stream: "
									+ messageStr);
